...
{
    private const string NamePrefix = "UrlEncodeHelper";

    [FunctionName(NamePrefix)]
    [OpenApiOperation(operationId: "Run", tags: new[] { "Helpers" })]
    [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
    [OpenApiParameter(name: "Input", In = ParameterLocation.Query, Required = true, Type = typeof(string), Description = "Text to encode.")]
        ...
        [HttpTrigger(AuthorizationLevel.Function, "get", Route = null)] HttpRequest req, ILogger log)
    {
        ...
        try
        {
            var output = new { Output = FormatHelper.UrlEncodeString(input)};
